      ...
      >
        ...
        <input
          ...
          className={`w-full bg-transparent font-mono text-char-950 placeholder:text-char-400/40 outline-none text-center ${
            grow ? "text-3xl py-1" : "text-[1.375rem] py-2.5 px-8"
          }`}
        />
      </div>
    </label>
...
function PayoutRow({ label, count, value }) {
  return (
    <div className="flex justify-between items-center bg-white border border-char-200 rounded-lg px-3.5 py-2.5">
      <span className="font-mono text-[0.95rem] text-char-950">
        {label}
      ...
      <div className="max-w-md mx-auto px-4 pt-3 pb-4">
        ...
        <header className="mb-3">
          ...
          <div className="flex items-baseline gap-2 flex-wrap">
            ...
            <span className="font-body text-[0.82rem] text-char-400">Kāneʻohe</span>
          </div>
        </header>

        {/* Inputs, 3:2 split, both columns equal height */}
        <section className="grid grid-cols-5 gap-2.5 mb-4 items-stretch">
        ...
        {/* Payout panel */}
        <section className="border border-char-200 rounded-xl bg-sand/50 px-3 pt-2.5 pb-3">
        ...
        <button
          onClick={clearAll}
          disabled={!hasInput}
          className="w-full mt-3 py-2.5 rounded-lg border border-char-700 font-display tracking-[0.15em] text-[0.875rem] uppercase text-char-950/60 disabled:opacity-30 active:bg-char-950/5 transition-colors"
        >
          Clear
        </button>
      </div>
